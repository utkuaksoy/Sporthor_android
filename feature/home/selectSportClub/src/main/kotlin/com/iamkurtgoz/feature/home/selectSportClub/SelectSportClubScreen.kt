...
@Composable
internal fun SelectSportClubScreen(
    navigateUp: () -> Unit,
    popBackStack: () -> Unit,
    navigateToEditTeamScreen: (HomeScreenEditTeamScreenNavigationModel) -> Unit, 
    viewModel: SelectSportClubViewModel = hiltViewModel(),
) {
    val state by viewModel.state.collectAsStateWithLifecycle()

    TrackedScreen("SelectSportClubScreen")

    LaunchedEffect(key1 = Unit) {
        viewModel.setEvent(SelectSportClubScreenContract.Event.Initialize)
    }

    viewModel.sideEffect.observeSideEffect { event ->
        when (event) {
            is SelectSportClubScreenContract.SideEffect.NavigateUp -> navigateUp()
            is SelectSportClubScreenContract.SideEffect.PopBackStack -> popBackStack()
            is SelectSportClubScreenContract.SideEffect.NavigateToEditTeamScreen -> navigateToEditTeamScreen(event.model)
        }
    }

    SelectSportClubScreenScaffold(
        state = state,
...
@Composable
private fun SelectSportClubScreenScaffold(
    ...
    AppThemeScaffold(
            ...
            AppToolbar.Toolbar(
                leftContent = {
                    AppToolbarFields.NavigateIcon(
                        onClick = {
                            setEvent.invoke(SelectSportClubScreenContract.Event.NavigateUp)
                        },
                    )
                },
                centerContent = {
                    AppToolbarFields.Title(
                        text = "Kulüp Seç", // TODO: Localize
                    )
                },
            )
        },
        ...
        state.alertDialogModel?.Alert {
            setEvent.invoke(SelectSportClubScreenContract.Event.DismissDialogs)
        }
