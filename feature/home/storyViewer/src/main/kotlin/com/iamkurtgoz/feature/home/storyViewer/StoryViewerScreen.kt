...
internal fun StoryViewerScreen(
    ...
    TrackedScreen("StoryViewerScreen")

    LaunchedEffect(key1 = Unit) {
        viewModel.setEvent(StoryViewerScreenContract.Event.Initialize)
    }

    viewModel.sideEffect.observeSideEffect { event ->
        when (event) {
            is StoryViewerScreenContract.SideEffect.NavigateUp -> navigateUp()
            is StoryViewerScreenContract.SideEffect.PopBackStack -> popBackStack()
        }
    }

    StoryViewerScreenScaffold(
        state = state,
...
@Composable
private fun StoryViewerScreenScaffold(
    ...
    AppThemeScaffold {
        ...
        state.alertDialogModel?.Alert {
            setEvent.invoke(StoryViewerScreenContract.Event.DismissDialogs)
        }
