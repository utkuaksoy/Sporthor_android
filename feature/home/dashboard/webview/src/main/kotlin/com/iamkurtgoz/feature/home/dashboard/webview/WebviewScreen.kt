...
internal fun WebviewScreen(
    ...
    TrackedScreen("WebviewScreen")

    LaunchedEffect(key1 = Unit) {
        viewModel.setEvent(WebviewScreenContract.Event.Initialize)
    }

    viewModel.sideEffect.observeSideEffect { event ->
        when (event) {
            is WebviewScreenContract.SideEffect.NavigateUp -> navigateUp()
            is WebviewScreenContract.SideEffect.PopBackStack -> popBackStack()
        }
    }

    WebviewScreenScaffold(
        state = state,
...
@Composable
private fun WebviewScreenScaffold(
    ...
    AppThemeScaffold(
            ...
            AppToolbar.Toolbar(
                leftContent = {
                    AppToolbarFields.NavigateIcon(
                        onClick = {
                            setEvent.invoke(WebviewScreenContract.Event.NavigateUp)
                        },
                    )
                },
                centerContent = {
                    AppToolbarFields.Title(
                        text = state.navigateRoute.routeType.title, // TODO: Localize
                    )
                },
            )
        },
        ...
        state.alertDialogModel?.Alert {
            setEvent.invoke(WebviewScreenContract.Event.DismissDialogs)
        }
