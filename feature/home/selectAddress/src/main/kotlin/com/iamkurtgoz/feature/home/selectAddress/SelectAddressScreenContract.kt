...
import com.iamkurtgoz.feature.home.selectAddress.domain.model.PlaceUIModel

internal class SelectAddressScreenContract {
    @Immutable
    data class State(
        ...
        val route: HomeScreenSelectAddressRoute,
        val textSearch: AppTextFieldValue = AppTextFieldValue(),
        val nearbyLocations: List<PlaceUIModel> = emptyList(),
        val filteredNearbyLocations: List<PlaceUIModel> = emptyList(),
    ...
    sealed class Event : CoreState.Event {
        ...
        data object LoadNearbyLocations : Event()
        data class SelectedAddress(val title: String, val address: String, val city: String?, val country: String?) : Event()
        data object FindSelectedAddress : Event()
        data class OnMapClick(val location: LatLng) : Event()
    }

    object Static {
        const val SEARCH_DEBOUNCE: Long = 500
        const val MIN_SEARCH_LENGTH: Int = 3
    }
}
