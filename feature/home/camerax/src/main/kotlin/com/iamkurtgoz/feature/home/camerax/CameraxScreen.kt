...
internal fun CameraxScreen(
    ...
    TrackedScreen("CameraxScreen")

    LaunchedEffect(key1 = Unit) {
        viewModel.setEvent(CameraxScreenContract.Event.Initialize)
    }

    viewModel.sideEffect.observeSideEffect { event ->
        when (event) {
            is CameraxScreenContract.SideEffect.NavigateUp -> navigateUp()
            is CameraxScreenContract.SideEffect.PopBackStack -> popBackStack()
        }
    }

    CameraxScreenScaffold(
        state = state,
...
@Composable
private fun CameraxScreenScaffold(
    ...
    AppThemeScaffold { padding ->
        ...
        state.alertDialogModel?.Alert {
            setEvent.invoke(CameraxScreenContract.Event.DismissDialogs)
        }
