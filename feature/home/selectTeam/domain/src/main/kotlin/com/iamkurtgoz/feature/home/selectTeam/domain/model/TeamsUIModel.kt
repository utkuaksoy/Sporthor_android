 ...
 */
package com.iamkurtgoz.feature.home.selectTeam.domain.model

import com.iamkurtgoz.domain.model.base.Listable

data class TeamsUIModel(
    val teams: List<TeamsUIItemModel?>?,
)
...
data class TeamsUIItemModel(
    ...
    val value: String?,
    val image: String?,
    override val uuid: String? = "$name$value$image",
) : Listable {
    fun isMatch(text: String): Boolean {
        return name?.contains(text, ignoreCase = true) == true ||
            value?.contains(text, ignoreCase = true) == true
    }
}
