...
internal fun LoginWithEmailScreen(
    ...
    TrackedScreen("LoginWithEmailScreen")

    LaunchedEffect(key1 = Unit) {
        viewModel.setEvent(LoginWithEmailScreenContract.Event.Initialize)
    }

    viewModel.sideEffect.observeSideEffect { event ->
        when (event) {
            is LoginWithEmailScreenContract.SideEffect.NavigateUp -> navigateUp()
            is LoginWithEmailScreenContract.SideEffect.PopBackStack -> popBackStack()
            is LoginWithEmailScreenContract.SideEffect.NavigateToForgotPassword -> navigateToForgotPassword()
            is LoginWithEmailScreenContract.SideEffect.NavigateToHome -> navigateToHome()
        }
    }

    LoginWithEmailScreenScaffold(
        state = state,
...
@Composable
private fun LoginWithEmailScreenScaffold(
    ...
    AppThemeScaffold(
        bottomBar = {
            AppButton.PrimaryLarge(
                ...
                onClick = {
                    setEvent.invoke(LoginWithEmailScreenContract.Event.Login)
                },
            )
        },
    ) { padding ->
        ...
        state.alertDialogModel?.Alert {
            setEvent.invoke(LoginWithEmailScreenContract.Event.DismissDialogs)
        }
