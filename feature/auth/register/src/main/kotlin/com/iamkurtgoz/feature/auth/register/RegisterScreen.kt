...
internal fun RegisterScreen(
    ...
    TrackedScreen("RegisterScreen")

    LaunchedEffect(key1 = Unit) {
        viewModel.setEvent(RegisterScreenContract.Event.Initialize)
    }

    viewModel.sideEffect.observeSideEffect { event ->
        when (event) {
            is RegisterScreenContract.SideEffect.NavigateUp -> navigateUp()
            is RegisterScreenContract.SideEffect.PopBackStack -> popBackStack()
            is RegisterScreenContract.SideEffect.NavigateToOtp -> navigateToOtp(event.model)
            is RegisterScreenContract.SideEffect.NavigateToLogin -> navigateToLogin()
            is RegisterScreenContract.SideEffect.HideKeyboard -> localKeyboardController?.hide()
            is RegisterScreenContract.SideEffect.ClearTextFieldFocus -> focusRequester.clearFocus(force = true)
        }
    }
    ...
    AppThemeScaffold(
        bottomBar = {
            AppButton.PrimaryLarge(
                text = stringResource(resourcesR.string.button_send_otp_code),
                modifier = Modifier
                    .fillMaxWidth()
                ...
                enabled = state.textPhoneNumber.value.isNotEmpty(),
                onClick = {
        ...
        state.alertDialogModel?.Alert {
            setEvent.invoke(RegisterScreenContract.Event.DismissDialogs)
        }
