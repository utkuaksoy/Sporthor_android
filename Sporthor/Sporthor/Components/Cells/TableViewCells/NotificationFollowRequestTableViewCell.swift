...
//  Sporthor
//
//  Created by GÜRHAN YUVARLAK on 2.08.2025.
//
import UIKit
import ComponentKit

protocol NotificationFollowRequestTableViewCellDelegate: AnyObject {
    func didTappedFollowRequestButton(
        _ model: NotificationModel,
        isAccepted: Bool
    )
}

extension NotificationFollowRequestTableViewCellDelegate {
    func didTappedFollowRequestButton(
        _ model: NotificationModel,
        isAccepted: Bool
    ) {}
...
final class NotificationFollowRequestTableViewCell: UITableViewCell {
    // MARK: - UI Elements
    ...
    // MARK: - Members
    private weak var delegate: NotificationFollowRequestTableViewCellDelegate?
    ...
    // MARK: - Bind
    func configure(
        delegate: NotificationFollowRequestTableViewCellDelegate? = nil,
...
extension NotificationFollowRequestTableViewCell: CKButtonDelegate {
    func ckButtonDidTap(tag: Int) {
        guard let model else { return }
        switch tag {
        case 1:
            delegate?.didTappedFollowRequestButton(model, isAccepted: true)
        case 2:
            delegate?.didTappedFollowRequestButton(model, isAccepted: false)
        default:
            break
        }
    }
