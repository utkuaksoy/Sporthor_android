...
final class ListClubViewController: BaseViewController {
    // MARK: - VIPER Variables
    var presenter: ListClubPresenterProtocol {
        get { return self.basePresenter as! ListClubPresenterProtocol }
...
// MARK: - ListClubPresenterDelegate
extension ListClubViewController: ListClubPresenterDelegate {
        ...
        DispatchQueue.main.async { [weak self] in
            ...
            if let navCon = navigationController as? CustomNavigationController {
                ...
                navCon.isTextRightBarButtonItem = (
                    text: isHidden ? presenter.sportClubs.isEmpty ? "" : "Sil" : "İptal",
...
extension ListClubViewController: UITableViewDataSource {
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return presenter.sportClubs.isEmpty ? 1 : presenter.sportClubs.count
    }
    
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if presenter.sportClubs.isEmpty {
            let cell = EmptyTableViewCell.dequeue(from: tableView, at: indexPath)
            cell.bind(image: Asset.infoAlert.image, description: "Kulübünüz bulunmamaktadır.")
            return cell
        } else {
            let model = presenter.sportClubs[indexPath.row]
            let cell = ListClubTableViewCell.dequeue(from: tableView, at: indexPath)
            cell.configure(with: model,
                           rightIcon: presenter.isDeleted ? model.isSelected ? Asset.checked.image : Asset.unchecked.image : Asset.chevronRightGrey.image)
            return cell
        }
    }
}
...
extension ListClubViewController: UITableViewDelegate {
...
extension ListClubViewController: CustomNavigationControllerDelegate {
    func didTapButton(type: BarButtonItemType) {
        presenter.didTappedNavigationButton(type)
    }
}

extension ListClubViewController: CKButtonDelegate {
