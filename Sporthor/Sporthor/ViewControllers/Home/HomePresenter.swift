...
final class HomePresenter: BasePresenter {
    ...
    weak var view: HomePresenterDelegate? {
        get { return self.baseView as? HomePresenterDelegate }
        set { self.baseView = newValue }
    }
    
    var interactor: HomeInteractorProtocol {
        get { return self.baseInteractor as! HomeInteractorProtocol }
        set { self.baseInteractor = newValue }
    }
    
    var router: HomeRouterProtocol {
        get { return self.baseRouter as! HomeRouterProtocol }
    ...
    private var snapshot: NSDiffableDataSourceSnapshot = NSDiffableDataSourceSnapshot<HomeSection, HomeItem>()
...
extension HomePresenter: HomePresenterProtocol {
    func viewDidLoad() {
        ...
        setTitle()
        view?.prepareUI()
        prepareSnapshotSection()
        prepareSnapshotHeader()
        if homeModeType == .home {
            getTask()
        } else {
            currentPage = posts.count / pageSize
            prepareSnapshotPosts()
        }
        view?.didApplySnapshot(snapshot)
    ...
    private func navigate(_ routes: HomeRoutes) {
    ...
    private func prepareSnapshotSection() {
        snapshot.appendSections([.header, .stories, .posts])
    }
    
    private func prepareSnapshotHeader() {
        guard homeModeType == .home else { return }
        snapshot.appendItems([
            .header(HeaderModel(
                headerIcon: Icons.headerApp,
    ...
    private func prepareSnapshotStories() {
        guard !stories.isEmpty else { return }
        snapshot.appendItems(stories.map({.story($0)}), toSection: .stories)
        view?.didApplySnapshot(snapshot)
    }
    
    private func prepareSnapshotPosts() {
    ...
    private func reloadAllSnapshot() {
        snapshot = NSDiffableDataSourceSnapshot<HomeSection, HomeItem>()
        prepareSnapshotSection()
        if homeModeType == .home {
            prepareSnapshotHeader()
            prepareSnapshotStories()
        }
        prepareSnapshotPosts()
    ...
    func didTappedActionButton(tag: Int, indexPath: IndexPath) {
        ...
        case 1:
            navigate(.comment(postId: posts[indexPath.item].id))
        default:
            break
        }
    }
...
extension HomePresenter: PreviewDelegate {
    func didAddStory() {
        getStories()
    }
}
...
// MARK: - StoryDelegate
extension HomePresenter: StoryDelegate {
...
extension HomePresenter: PostSettingDelegate {
    func dismissHideOrRemove(model: Post) {
        let item = HomeItem.post(model)
