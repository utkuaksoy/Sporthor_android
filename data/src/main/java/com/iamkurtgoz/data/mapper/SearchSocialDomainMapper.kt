...
internal class SearchSocialDomainMapper @Inject constructor() : IMapper<SearchSocialResponseModel, SearchSocialDomainModel> {
    override fun map(response: SearchSocialResponseModel): SearchSocialDomainModel {
        return with(response) {
            SearchSocialDomainModel(
                searchList = searchList?.map {
                    SocialDomainItemModel(
                        id = it?.id,
                        image = it?.image,
                        name = it?.name,
