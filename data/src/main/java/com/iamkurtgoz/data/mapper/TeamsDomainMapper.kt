...
internal class TeamsDomainMapper @Inject constructor() : IMapper<TeamsResponseModel, TeamsDomainModel> {
    override fun map(response: TeamsResponseModel): TeamsDomainModel {
        return with(response) {
            TeamsDomainModel(
                teams = teams?.map {
                    TeamsDomainItemModel(
                        name = it?.name,
                        value = it?.value,
                        image = it?.image,
                        detail = it?.detail,
